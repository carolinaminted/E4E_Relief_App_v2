...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ applications, onChatbotAction, isOpen, setIsOpen, scrollContainerRef, activeFund }) => {
  ...
  return (
    <>
      <div 
        className={`fixed top-24 bottom-40 w-full max-w-sm max-h-[600px] bg-[#ff8400]/90 backdrop-blur-sm rounded-lg shadow-2xl flex flex-col z-50 border border-orange-800/50 transition-all duration-300 ease-in-out left-1/2 -translate-x-1/2 md:top-auto md:bottom-24 md:h-[calc(100vh-8rem)] md:left-8 md:-translate-x-0 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
        aria-hidden={!isOpen}
      >
       <header className="bg-black/40 p-4 border-b border-black/30 shadow-lg rounded-t-lg flex-shrink-0">
        <div>
            <h1 className="text-lg font-bold text-white">
              Relief Assistant
            </h1>
            <p className="text-xs text-orange-300 italic mt-1">*AI Agent preview using generative responses</p>
        </div>
      </header>
       <main className="flex-1 overflow-hidden flex flex-col">
        <ChatWindow messages={messages} isLoading={isLoading} />
      </main>
      <footer className="p-4 bg-black/40 border-t border-black/30 rounded-b-lg flex-shrink-0">
